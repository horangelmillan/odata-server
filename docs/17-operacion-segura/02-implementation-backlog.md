...
| R4 | Riesgo | **Migraciones sin lock entre procesos** | `migrator.ts` ejecuta `umzug.up()` en cada arranque; 2+ réplicas simultáneas compiten por `SequelizeMeta` (una puede fallar con tabla/columna existente). Solo relevante al escalar | Baja | Descartado | D1 (single-instance; nota en runbook §escalado para el patrón futuro: migraciones como paso de despliegue) |
| R5 | Riesgo | **Rate-limit in-memory por instancia** | `main.ts` usa `express-rate-limit` sin store compartido; con N réplicas el límite 100/15min/IP se multiplica por N. Solo relevante al escalar | Baja | Descartado | D1 (single-instance; nota en runbook §escalado) |
...
| R8 | Riesgo | **`uuid@8.3.2` moderada transitiva** | `pnpm audit --prod`: 1 moderada vía `sequelize@6.37.8 > uuid@8.3.2`; **0 usos directos en `src/`** (grep verificado); advisory GHSA-w5hq-g745-h8pq cubre `<11.1.1` | Baja | Implementado | F4 (override `uuid@^11.1.1` — ESM-only verificado con Sequelize 6 en Node 22.20: suite 215/215 + smoke) |
...
| M4 | Mejora | **Sin runbook de operación** | No existe documentación de despliegue (compose prod), secrets, SSL, backups, rollout ni troubleshooting | Media | Implementado | F5 (runbook.md completo: despliegue, secrets+rotación, backup/restore, SSL, dependencias, rollout, troubleshooting, escalado) |
| DT1 | Deuda Técnica | **bcrypt → node-pre-gyp → tar (13 avisos)** | `pnpm audit`: 13 avisos (1 critical, 7 high, 5 moderate) en la cadena transitiva de **build/install** de `bcrypt@5.1.1` (`@mapbox/node-pre-gyp` → `tar`/`rimraf`/`minimatch`/`brace-expansion`). No se ejecuta en runtime (el server usa el binario compilado); mitigado por `--frozen-lockfile` + lockfile versionado | Baja | Descartado | D4 (mitigación documentada en runbook §dependencias; re-evaluar al actualizar bcrypt) |
| DT2 | Deuda Técnica | **SECRET_KEY estática con rotación manual** | JWT con TTL 8h sin refresh; la rotación de la clave no está documentada (cambiar la clave invalida todos los tokens) | Baja | Implementado | F5 (procedimiento de rotación en runbook §Secrets) |
| IF1 | Investigación | **Observabilidad externa** | Reabierto desde ciclo 16 ("Movido a iniciativa futura"): métricas (prom-client), APM, logging estructurado con transporte — sin operación real que lo requiera | Baja | Descartado | D2 (YAGNI: `/healthz` + morgan cubren el mínimo; nota de reapertura en runbook §escalado) |
| DA1 | Decisión | **Modelo single-instance** | El despliegue objetivo es una instancia; R4/R5 aceptados con nota (ver `01-arquitectura-propuesta.md` D1) | — | Implementado | D1 (registrada en F0; R4/R5 cerrados en F6) |
| DA2 | Decisión | **Observabilidad: cerrar IF1** | Sin consumidor de métricas no aporta valor; se reabre con despliegue real | — | Implementado | D2 (registrada en F0; IF1 cerrado en F6) |
| DA3 | Decisión | **Estrategia de backup** | pg_dump + retención + restore verificado; sin cron externo ni SaaS | — | Implementado | D3 (registrada en F0; ejecutada en F1) |
| DA4 | Decisión | **bcrypt: mantener + documentar** | Sin mejora de seguridad real en el hash; mitigación build-time documentada | — | Implementado | D4 (registrada en F0; DT1 cerrado en F4/F5) |
| DA5 | Decisión | **uuid: override a ^11.1.1** | El advisory (GHSA-w5hq-g745-h8pq) cubre `<11.1.1`; 9.x no bastaba. uuid 11 es ESM-only — verificado compatible con Sequelize 6 en Node 22.20 | — | Implementado | D5 (registrada en F0; ejecutada en F4) |
| DA6 | Decisión | **Bump 2.3.1** | Patch: el ciclo toca código productivo | — | Implementado | D6 (registrada en F0; ejecutada en F6) |
...
| 2026-08-01 | R8, DT1, DA5 | F4 ejecutada: override `uuid@^11.1.1` (Context7: sintaxis pnpm.overrides; el advisory cubre <11.1.1, 9.x no bastaba; uuid 11 ESM-only verificado con Sequelize 6 en Node 22.20: suite 215/215 + smoke). Audit: de 14 a **13 advisories, todos de la cadena de build de bcrypt** (DT1 → Descartado vía D4; filtro del CI simplificado) |
| 2026-08-01 | M4, DT2 | F5 ejecutada: runbook completo (despliegue compose prod, secrets + rotación SECRET_KEY, backup/restore, SSL/DB_SSL, dependencias/audit, rollout, troubleshooting, notas de escalado D1). Docs-only — sin cambios de código |
| 2026-08-01 | R4, R5, IF1, DA1–DA6, D6 | F6 ejecutada: validación integral (build ✅, tsc ✅, **215/215** ✅, smoke dist dev ✅ healthz/metadata, smoke dist prod ✅ healthz/401/login→200, audit 13 conocidos) + bump `2.3.1` (D6). Cierre: R4/R5 → Descartado (D1), IF1 → Descartado (D2), DA1–DA6 → Implementado. Backlog 17 sin "Pendiente"/"En evaluación". Commit + push + PR + CI + merge + tag `v2.3.1` |
