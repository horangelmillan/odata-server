...
> **Inicio:** 2026-07-16
> **Estado global:** ✅ F0 lista para continuar — [issue #3](https://github.com/horangelmillan/odata-server/issues/3) resuelto y mergeado a `master` vía [PR #4](https://github.com/horangelmillan/odata-server/pull/4).
...
|---|---|---|---|---|
| **F0** | Rama `feature/financial-eco`; baseline de tests (143 pass); plan maestro; **aplicar prefijo `demo/` a `product`/`category`** (getEndpoint → `/odata/demo/...`). | Rama creada; tests verdes (143 pass baseline); docs del ciclo 06 iniciadas. **✅ F0.1 OK**: acoplamiento ruta↔modelo resuelto y mergeado vía [PR #4](https://github.com/horangelmillan/odata-server/pull/4). F0.1 retomado: prefijo `demo/` aplicado + tests migrados. | Bajo | `fases/f0-ramificacion-baseline.md` · `fases/f0.1-prefijo-demo-bloqueado.md` |
