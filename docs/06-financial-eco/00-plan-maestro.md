...
> **Inicio:** 2026-07-16
> **Estado global:** 🔄 En revisión — F0 bloqueada hasta merge de [PR #4](https://github.com/horangelmillan/odata-server/pull/4) (acoplamiento ruta↔modelo resuelto, pendiente de merge a `master`).
...
|---|---|---|---|---|
| **F0** | Rama `feature/financial-eco`; baseline de tests (143 pass); plan maestro; **aplicar prefijo `demo/` a `product`/`category`** (getEndpoint → `/odata/demo/...`). | Rama creada; tests verdes (143 pass baseline); docs del ciclo 06 iniciadas. **🔄 EN REVISIÓN**: acoplamiento ruta↔modelo resuelto vía [PR #4](https://github.com/horangelmillan/odata-server/pull/4), pendiente merge a `master`. F0.1 retoma tras el merge. | Bajo | `fases/f0-ramificacion-baseline.md` · `fases/f0.1-prefijo-demo-bloqueado.md` |
