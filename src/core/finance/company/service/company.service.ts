...
import { CompanyCreateDTO, CompanyUpdateDTO } from "../dto/company.dto.js";
...
function modelOf(controller: CompanyODataController): ODataBaseModel {
...
class CompanyService {
    private controller = new CompanyODataController();
    ...
    async create(data: unknown): Promise<WriteResult> {
        const dto = await validate(CompanyCreateDTO, data);
        const model = modelOf(this.controller);
    ...
    async update(id: string, data: unknown): Promise<WriteResult> {
        const dto = await validate(CompanyUpdateDTO, data);
        const model = modelOf(this.controller);
        return await odataWriteService.runInTransaction((tx) =>
