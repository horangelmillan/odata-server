...
import { SupplierInvoiceCreateDTO, SupplierInvoiceUpdateDTO } from "../dto/supplierinvoice.dto.js";
...
function modelOf(controller: SupplierInvoiceODataController): ODataBaseModel {
...
class SupplierInvoiceService {
    private controller = new SupplierInvoiceODataController();
    ...
    async create(data: unknown): Promise<WriteResult> {
        const dto = await validate(SupplierInvoiceCreateDTO, data);
        const model = modelOf(this.controller);
    ...
    async update(id: string, data: unknown): Promise<WriteResult> {
        const dto = await validate(SupplierInvoiceUpdateDTO, data);
        const model = modelOf(this.controller);
        return await odataWriteService.runInTransaction((tx) =>
...
const supplierInvoiceService: SupplierInvoiceService = new SupplierInvoiceService();
export { supplierInvoiceService };
