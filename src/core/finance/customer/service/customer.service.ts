...
function modelOf(controller: CustomerODataController): ODataBaseModel {
...
class CustomerService {
    private controller = new CustomerODataController();
    ...
    async create(data: unknown): Promise<WriteResult> {
        const dto = await validate(CustomerCreateDTO, data);
        const model = modelOf(this.controller);
    ...
    async update(id: string, data: unknown): Promise<WriteResult> {
        const dto = await validate(CustomerUpdateDTO, data);
        const model = modelOf(this.controller);
        return await odataWriteService.runInTransaction((tx) =>
