...
import { SupplierPaymentCreateDTO, SupplierPaymentUpdateDTO } from "../dto/supplierpayment.dto.js";
...
class SupplierPaymentService {
    private controller = new SupplierPaymentODataController();
    ...
    async create(data: unknown): Promise<WriteResult> {
        const dto = await validate(SupplierPaymentCreateDTO, data);
        const model = modelOf(this.controller);
    ...
    async update(id: string, data: unknown): Promise<WriteResult> {
        const dto = await validate(SupplierPaymentUpdateDTO, data);
        const model = modelOf(this.controller);
        return await odataWriteService.runInTransaction((tx) =>
...
const supplierPaymentService: SupplierPaymentService = new SupplierPaymentService();
export { supplierPaymentService };
