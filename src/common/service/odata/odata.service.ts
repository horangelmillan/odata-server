...
// RF1 (ciclo 16, F1): el router OData ya no es un singleton que importa
// `domainRegistrations` (core) ni el datasource. `createODataExpressApp` recibe
// la composición desde el bootstrap (`src/main.ts`) y `common` queda sin
// imports de `core`.
export function createODataExpressApp(
    registrations: DomainRegistration[],
    dataSource: DataSource,
): Router {
    const oDataExpressApp: Router = Router();

    const controllers: ODataControler[] = registrations.map(r => r.controller);
